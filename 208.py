...
def greaterthan5(l):
    # checks if any item in the list is greater than 5
    for item in l:
        if item > 5:
            return True
    return False
...
def generate(moves, l):
    ...
    elif greaterthan5(addtoclass(moves)) == False:
        # False here means every class is <= 5
        
        # make a copy of the current moves
        temp = list(moves)
        # add a right turn and a left turn and rerun the code
        temp.append(1)
        moves.append(-1)
        generate(moves, l)
        generate(temp, l)
